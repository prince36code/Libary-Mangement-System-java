...
git pull -> get the latest changes  that where made


// the Stating process 
-> get the repo from your repository using git clone https://github.com/username/my-project.git
->This download the files and all its commits into local repo in a folder my-project
...
git status
→ Shows file status (untracked, modified, staged)
...
git add .
→ Adds all files to the staging area (prepares them for commit)

git commit -m "Initial commit"
→ Saves a snapshot of the staged files with a message

git branch -M main
→ Renames the default branch to main

git remote add origin https://github.com/USERNAME/REPO_NAME.git
→ Connects your local repo to the GitHub remote repo

git push -u origin main
→ Uploads commits to GitHub and links local main to remote main
